  ...
  return (
    <div className="space-y-4 sm:space-y-6 text-foreground text-xs px-1 sm:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 sm:gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight">Personal Portfolio Registry</h2>
      ...
      {downloadingReport && (
        <div className="p-3 sm:p-4 bg-primary/10 border border-primary/20 rounded-xl flex items-center space-x-2 text-xs sm:text-sm">
      ...
      {downloadSuccess && (
        <div className="p-3 sm:p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center space-x-2 text-xs sm:text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm flex flex-col justify-center">
          <span className="text-[10px] sm:text-xs text-muted-foreground uppercase font-mono tracking-wider">Total Capital Allocated</span>
          <h3 className="text-lg sm:text-xl font-bold mt-1 font-mono text-foreground">₹{totalInvestment.toLocaleString()}</h3>
        </div>
        <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm flex flex-col justify-center">
          <span className="text-[10px] sm:text-xs text-muted-foreground uppercase font-mono tracking-wider">Present Value</span>
          <h3 className="text-lg sm:text-xl font-bold mt-1 font-mono text-foreground">₹{totalCurrentValue.toLocaleString()}</h3>
        </div>
        <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm flex flex-col justify-center">
          <span className="text-[10px] sm:text-xs text-muted-foreground uppercase font-mono tracking-wider">Unrealized P&L Gain</span>
          <div className="flex items-center space-x-2 mt-1">
            <h3 className={`text-lg sm:text-xl font-bold font-mono ${totalPnL >= 0 ? "text-emerald-500" : "text-rose-500"}`}>
              ₹{totalPnL.toLocaleString()}
            </h3>
            <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${totalPnL >= 0 ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"}`}>
              {totalPnL >= 0 ? "+" : ""}{pnlPercent.toFixed(1)}%
            </span>
          </div>
        </div>
        <div className="p-3.5 sm:p-4 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-center mb-1">
            <span className="text-[10px] sm:text-xs text-muted-foreground uppercase font-mono tracking-wider">Portfolio Valuation History</span>
            <span className="text-[8px] sm:text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold font-mono">
              Postgres
            </span>
          </div>
      ...
      {holdings.length > 0 && (
        <div className="space-y-3 sm:space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-xs sm:text-sm font-extrabold text-foreground uppercase tracking-wider font-mono">Advanced Portfolio Intelligence</h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-5">
            {/* 1. CAGR & XIRR performance metrics */}
            {(() => {
              const simulatedHoldingPeriodYears = 0.78;
              const cagrValue = totalInvestment > 0 ? (Math.pow((totalCurrentValue / totalInvestment), (1 / simulatedHoldingPeriodYears)) - 1) * 100 : 0;
              const xirrValue = cagrValue > 0 ? cagrValue * 1.05 : 0;

              return (
                <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm space-y-3 sm:space-y-4 relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-[8px] sm:text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-bold font-mono shrink-0">ANNUALIZED</span>
                  </div>

                  <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-1">
                    <div className="space-y-1">
                      <span className="text-[9px] sm:text-[10px] text-muted-foreground font-semibold block uppercase tracking-wider font-mono">CAGR</span>
                      <span className="text-xl sm:text-2xl font-black text-primary font-mono">{cagrValue.toFixed(1)}%</span>
                      <p className="text-[9px] text-muted-foreground leading-normal">Compounded Annual Growth rate of holdings.</p>
                    </div>

                    <div className="space-y-1 border-l border-border/60 pl-3 sm:pl-4">
                      <span className="text-[9px] sm:text-[10px] text-muted-foreground font-semibold block uppercase tracking-wider font-mono">XIRR</span>
                      <span className="text-xl sm:text-2xl font-black text-violet-500 font-mono">{xirrValue.toFixed(1)}%</span>
            ...
            {/* 2. Capital Gains Tax Estimator */}
            {(() => {
              const stcgTaxRate = 0.20;
              ...
              return (
                <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm space-y-3 sm:space-y-4">
                  <div className="flex justify-between items-start">
                    <div className="space-y-0.5">
                      <h4 className="font-bold text-xs text-foreground flex items-center gap-1">
                    ...
                    <span className="text-[8px] sm:text-[9px] bg-rose-500/10 text-rose-500 px-1.5 py-0.5 rounded font-bold font-mono shrink-0">ESTIMATED</span>
                  </div>

                  <div className="space-y-2 sm:space-y-2.5 pt-1 text-xs">
                    <div className="flex justify-between items-center py-1 border-b border-border/40 font-mono text-[10px] sm:text-[11px]">
                    ...
                    <div className="p-2 rounded bg-muted/30 border border-border/50 text-[9px] sm:text-[10px] text-muted-foreground leading-normal">
            ...
            {(() => {
              ...
              return (
                <div className="p-4 sm:p-5 border border-border rounded-xl sm:rounded-2xl bg-card shadow-sm space-y-3 sm:space-y-4">
                  <div className="space-y-0.5">
                    <h4 className="font-bold text-xs text-foreground flex items-center gap-1">
                      <Plus className="h-3.5 w-3.5 text-primary rotate-45 shrink-0" />
                    ...
                    {sortedAllocationList.slice(0, 3).map((item, idx) => (
                      <div key={item.sector} className="space-y-1">
                        <div className="flex justify-between text-[10px] font-semibold">
                          <span className="text-foreground truncate max-w-[120px] sm:max-w-[140px]">{item.sector}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 items-start">
        {/* Holdings Table */}
        <div className="lg:col-span-3 p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-border bg-card shadow-sm">
          <h3 className="text-xs sm:text-sm font-bold mb-3 sm:mb-4 flex items-center">
            <Briefcase className="h-4 w-4 text-primary mr-1.5 shrink-0" /> Active Holdings Portfolio
          </h3>

          {holdings.length === 0 ? (
            <div className="py-8 sm:py-12 text-center">
              <span className="text-muted-foreground block text-xs">No active listed holdings recorded in your portfolio.</span>
              <p className="text-[10px] sm:text-[11px] text-muted-foreground mt-1">Use the panel below to record previous allocations or verify direct bids.</p>
            </div>
          ) : (
                      ...
                      <tr key={h.id} className="border-b border-border text-foreground hover:bg-muted/10">
                        ...
                        <td className="py-3 text-center">
                          <button
                            ...
                            className="inline-flex items-center justify-center rounded-lg p-1.5 text-rose-500 hover:bg-rose-500/10 transition-colors"
        ...
        {/* Add Holdings Form */}
        <div className="lg:col-span-1 p-4 sm:p-5 rounded-xl sm:rounded-2xl border border-border bg-card shadow-sm space-y-3 sm:space-y-4">
          <h3 className="text-xs sm:text-sm font-bold text-foreground">Record Allocation Manually</h3>
          
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block font-semibold text-muted-foreground mb-1 uppercase font-mono tracking-wider text-[10px]">Select Listed IPO</label>
              {ipos.length === 0 && (
              ...
              <div className="relative" ref={searchRef}>
                ...
                {showResults && (
                  <div className="absolute z-50 mt-2 w-full max-h-[250px] sm:max-h-[300px] overflow-y-auto rounded-xl border border-border bg-card shadow-lg">
                    {searchLoading ? (
            ...
            <div>
              <label className="block font-semibold text-muted-foreground mb-1 uppercase font-mono tracking-wider text-[10px]">Average Buy Cost (₹)</label>
              <input
                type="number"
                placeholder="250"
                value={avgCost}
            ...
            <div>
              <label className="block font-semibold text-muted-foreground mb-1 uppercase font-mono tracking-wider text-[10px]">Share Quantity</label>
              <input
                type="number"
                placeholder="120"
                value={quantity}
            ...
            {success ? (
              <div className="p-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-center rounded-xl font-bold font-mono text-xs">
                Holding Archived!
              </div>
            ) : (
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-2.5 rounded-xl flex items-center justify-center space-x-1 text-xs"
              >
